...
class FT_Dataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        _input, _input_len = padding_tokens(conditions + completion, self.max_seq_length, 0, 1)  # input+output+[pad to 512]

        pad_targets = [0 for i in range(0, self.prefix_len)] + context + [0 for i in range(0, self.infix_len)] + completion
        _target, _ = padding_tokens(pad_targets[1:], self.max_seq_length, 0, 1)  # input+output+[pad to 512], 和input一样，但是舍弃了第一个字符？？？
