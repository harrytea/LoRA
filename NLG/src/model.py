...
class Conv1D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.addmm(self.bias, x.view(-1, x.size(-1)), self.weight)  # 后两个参数相乘与第一个参数相加
        x = x.view(*size_out)
        return x
...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, len_kv=None):
        ...
        b = self.bias[:, :, ns-nd:ns, :ns]  # 下三角矩阵，这里是干啥的，不太清楚
        w = w * b - 1e10 * (1 - b)
...
class GPT2Model(nn.Module):
    def __init__(self, config):
        ...
        self.n_layer = config.n_layer  # 24
        self.n_embd = config.n_embd  # 1024
        self.n_vocab = config.vocab_size  # 50257
    ...
    def forward(
        ...
        input_shape = input_ids.size()  # [8, 512]
        input_ids = input_ids.view(-1, input_ids.size(-1))  # [8, 512]
        position_ids = position_ids.view(-1, position_ids.size(-1))  # [8, 512]

        inputs_embeds = self.wte(input_ids)  # [8, 512, 1024]
        position_embeds = self.wpe(position_ids)  # [8, 512, 1024]
